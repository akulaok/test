...
function SortingList({ sortingType, onChangeSortingType }: SortingListProps): JSX.Element {
  const [sortOptionsOpened, setSortOptionsOpened] = useState(false);

  return (
    <div className={styles.shadow}>
      <div className={styles.wrapper} onClick={() => setSortOptionsOpened(!sortOptionsOpened)}>
      ...
      {sortOptionsOpened &&
        <ul className={styles.sort_list}>
          {SortTypesArr.map((type) =>
            <li
              className={type === sortingType ? [styles.list__item, styles.list__item_active].join(' ') : styles.list__item}
              onClick={() => {
